// pages/medical-researcher/questionnaire/questionnaire.js
Page({
  data: {
    questionnaires: [], // 问卷列表
  ...
  viewQuestionnaire(e) {
    ...
    wx.navigateTo({
      url: `/pages/medical-researcher/questionnaire/preview/preview?id=${id}`
    });
  },
  ...
  editQuestionnaire(e) {
    const id = e.currentTarget.dataset.id;
    wx.navigateTo({
      url: `/pages/medical-researcher/questionnaire/create/create?id=${id}`
    });
  },
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          // 更新页面数据
          const newList = that.data.questionnaires.filter(item => item.id !== id);
          that.setData({
            questionnaires: newList,
            originalQuestionnaires: newList,
