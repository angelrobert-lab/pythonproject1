// pages/medical-researcher/questionnaire/create/create.js
Page({
  // 回到顶部
  backToTop() {
    wx.pageScrollTo({
  ...
  data: {
    ...
    showModuleDialog: false, // 是否显示模块对话框
    newModuleName: '', // 新模块名称
  ...
  onShow() {
    ...
    if (this.data.selectedDrawType) {
      ...
      if (this.data.currentModuleIndex >= 0) {
        modules[this.data.currentModuleIndex].questionCount += 1;
      }
  ...
  confirmAddModule() {
    ...
    const currentModuleIndex = modules.length - 1;
    ...
    console.log('添加模块:', newModule);
    console.log('当前模块索引:', currentModuleIndex);
  },
  ...
  previewQuestionnaire() {
    ...
    // 跳转到预览页面
    wx.navigateTo({
      url: '/pages/medical-researcher/questionnaire/preview/preview?id=' + previewQuestionnaire.id
    });
  },
  ...
  saveQuestionnaire() {
    ...
    const questionnairePage = pages.find(page => page.route === 'pages/medical-researcher/questionnaire/questionnaire');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const pages = getCurrentPages();
        // 查找问卷管理页面在栈中的位置
        const questionnairePageIndex = pages.findIndex(page => page.route === 'pages/medical-researcher/questionnaire/questionnaire');
